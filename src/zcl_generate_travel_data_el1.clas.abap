CLASS zcl_generate_travel_data_EL1 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_travel_data_EL1 IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA:itab TYPE TABLE OF ztravel_TAB_EL1.
...
*   delete existing entries in the database table
    DELETE FROM ztravel_TAB_EL1.

*   insert the new table entries
    INSERT ztravel_TAB_EL1 FROM TABLE @itab.

*   check the result
    SELECT * FROM ztravel_TAB_EL1 INTO TABLE @itab.
    out->write( sy-dbcnt ).
